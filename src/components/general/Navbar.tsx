...
const Navbar = () => {
  ...
  const navList = (
    <div className="mb-4 mt-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:justify-between lg:gap-6 text-white">
      <Link className="font-semibold text-lg" to="/#home" onClick={() => handleClickScroll('home')}>
        Home
      </Link>
      <Link className="font-semibold text-lg" to="/#about" onClick={() => handleClickScroll('about')}>
        About me
      </Link>
      <Link className="font-semibold text-lg" to="https://drive.google.com/file/d/1HNMKSw1xDAdhK_ighiqQAs6TyaZLIe0a/view?usp=share_link" target="_blank">
        Resume
      </Link>
      <Link className="font-semibold text-lg" to="/#projects" onClick={() => handleClickScroll('projects')}>
        Projects
      </Link>
      <Link className="font-semibold text-lg" to="/#contact" onClick={() => handleClickScroll('contact')}>
        Contact
      </Link>
    </div>
  );
  return (
    <Box className="fixed w-full top-0 left-0 z-[99] bg-bg_color text-white" ref={headerRef}>
      <Box mx="4" my="4">
